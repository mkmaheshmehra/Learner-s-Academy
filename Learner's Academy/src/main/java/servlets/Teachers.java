...
public class Teachers extends HttpServlet {
    public void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try{
            ...
            String query = "select teachers.Teacher_Name, teachers.Qualification, subjects.Subject_Name, class.Class" +
                    " from teachers inner join subjects on teachers.Subject_ID = subjects.Subject_ID inner join class" +
                    " on subjects.Class_ID = class.Class_ID";
            ResultSet resultSet = statement.executeQuery(query);
            out.println("<html><body><table style='border: 1px solid black;'><tr>");
            out.println("<th style='border: 1px solid black;'>Teacher's Name</th>");
            out.println("<th style='border: 1px solid black;'>Qualification</th>");
